...
After determining the best-performing model on the validation set (KNN), we used that model to generate predictions on the final test set (2000–2005), which contains completely unseen seasons. The idea here is to simulate a real-world scenario: if we had built our model before 2000, how well would it predict player development in the upcoming seasons? 
