...
We implemented 3 models, Random forest, Multiple Linear Regression and KNN.
...
To prepare our data for modeling, we applied a rigorous preprocessing pipeline. First of all, we converted all columns to numeric types and filled missing values with 0. Since the dataset spans historical eras where specific stats (like 3-pointers or blocks) were not recorded, a value of 0 represents a "non-recorded" event more accurately than a mean or median.Furthermore, we addressed the issue of players being traded mid-season by grouping records by ilkid (player ID) and year, summing the statistics to create a single, consolidated performance record for each player per season. Then, we calculated a custom Efficiency (EFF) metric using the standard NBA formula (PTS + REB + AST + STL + BLK - MissedFG - MissedFT - TO) and then we generated our target variable, Next_Year_EFF, by shifting the efficiency values backward by one year, effectively aligning current performance with future output.

Moreover, by dividing our  dataset into three strictly distinct time periods, we were trying to avoid overlapping our train, validation and test sets. For model learning, we used the data from 1946-1994. Validation set for tuning hyperparameters and for choosing a model, we utilized the data from 1995-1999. The last, but least test set for final evaluation contained the data from 2000-2004. The reason why we used strict temporal splitting is because randomly shuffling the data would have introduced data leakage. If the model was allowed to see a veteran player's stats in 1998 during training, it would implicitly "learn" the trajectory of his rookie season in 1990. By strictly separating the epochs, we forced the model to learn universal patterns of development rather than memorizing specific player careers.

### Random forest

To capture the complex trajectories of player development, we implemented a Random Forest, a powerful ensemble learning method. Unlike a linear model that applies a single mathematical formula to the entire dataset, Random Forest constructs a multitude of "Decision Trees" during training and outputs the average prediction of the individual trees. Firstly, we did not rely on default settings. Instead, we optimized the model structure using GridSearchCV to test different architectural combinations. We specifically tuned two critical hyperparameters: n_estimators (30, 60, 100) and max_depth (5, 10, 15), which create 9 configurations. Estimators control the number of trees and we tested a range of them to find the balance between stability and computational cost. On the contrary, maximum depth limits how deep each tree can grow. Too small depth(5) might underfit, while too deep growth can overfit, so we chose the optimal variant (n_estimators = 100, max_depth = 10).

Secondly, as we mentioned before, we used PredefinedSplit instead of cross-validation to avoid predicting the past using the future. We explicitly instructed the grid search to train ONLY on the 1946–1994 epoch and validate ONLY on the 1995–1999 epoch. Thirdly, our model was fed raw numeric inputs (Points, Minutes, Efficiency, etc.). Unlike Linear Regression or KNN, Random Forest does not require feature scaling (normalization), as decision trees make splits based on absolute thresholds (e.g., "Minutes < 2000") rather than distance calculations. Unlike Linear Regression, which assumes constant growth, Random Forest captures complex "aging curves" (e.g., rapid rookie improvement vs. veteran decline) by using decision boundaries. Additionally, the ensemble approach (averaging 100 trees) provided stability against the high variance of sports statistics, ultimately delivering the lowest Mean Absolute Error on the validation set.

### Multiple Linear Regression

To establish a robust performance baseline, we implemented a Multiple Linear Regression model augmented with Ridge Regularization (L2 penalty). Unlike the Random Forest, which can handle raw data, linear models are sensitive to the scale of input features. Therefore, our implementation began with a critical preprocessing step: applying a StandardScaler to normalize all numeric features. Without this normalization, the regularization term would have unfairly penalized features with larger raw values, distorting the model’s coefficients. For the modeling phase, we utilized scikit-learn's Ridge class rather than the standard LinearRegression. We configured the regularization strength with alpha=10, a hyperparameter chosen to constrain the magnitude of the model's coefficients. During training, the algorithm optimized the weights for each feature to minimize the sum of squared errors while simultaneously keeping the coefficients small. This dual objective allowed the model to learn the linear relationships between current stats and future efficiency without becoming unstable due to the high correlation between inputs. We specifically chose Ridge over standard Ordinary Least Squares (OLS) because our features exhibited high multicollinearity (e.g., Points and Minutes are > 70% correlated). Ridge regularization constrained the model coefficients, preventing overfitting and creating a stable linear benchmark to compare against our non-linear most optimal candidate. 


### KNN 

 In the second part of the analysis, we applied a K-Nearest Neighbors (KNN) regression model to predict how efficiently a player will perform in the following season. Unlike Random Forest or Linear Regression, KNN makes predictions by comparing each player to the most similar players from previous seasons. Because KNN relies heavily on distance calculations, we first standardized all numerical features using StandardScaler to ensure that variables measured on different scales (minutes, rebounds, points, etc.) contributed fairly. To find the most suitable configuration, we performed a Grid Search using different combinations of k (3, 5, 10, 20\) and weighting strategies (“uniform” vs. “distance”). The search was performed with a K-Fold cross-validation on the training set only. This allowed us to evaluate multiple versions of KNN and identify the one that consistently produced the smallest validation error. 
 
 After testing all candidates, the model with the lowest MAE on the internal CV was selected as the final KNN model. We then evaluated this model on the validation set (1995–1999) to check how well it generalizes to unseen data. We also visualized the predictions by plotting actual vs. predicted values, inspecting residual patterns, and examining the distribution of errors. These visual checks help us understand whether the model systematically over- or underestimates certain players and whether the prediction errors follow a stable pattern.
...
After determining the best-performing model on the validation set (Random Forest), we used that model to generate predictions on the final test set (2000–2005), which contains completely unseen seasons. The idea here is to simulate a real-world scenario: if we had built our model before 2000, how well would it predict player development in the upcoming seasons? 
